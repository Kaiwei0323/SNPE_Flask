...
class RtspPipeline:
    def __init__(self, uri, image_queue, capture_lock):
        ...
        if not all([self.rtspsrc, self.depay, self.h264parse, self.decoder, self.queue, self.capsfilter_nv12, self.videoconvert, self.videoscale, self.capsfilter_rgb, self.videorate, self.appsink]):
